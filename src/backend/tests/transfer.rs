...
prop_compose! {
    fn arb_backend_with_enough()(
    ...
    ) -> (Address, Address, u64, TestLedgerClient<(),(), InMemoryStorage<()>>, Vec<PolicyId>) {
        ...
        let mut total: u64 = 0;
        let mut builder = TestLedgerClientBuilder::<(), ()>::new(&signer);
...
proptest! {
    ...
    fn prop_can_transfer_funds_if_enough_balance(
        (signer, recipient, amount, ledger_client, decoys) in arb_backend_with_enough(),
    ) {
        let rt = Runtime::new().unwrap();

        rt.block_on(inner_test(signer, recipient, amount, ledger_client, decoys))
    }
}
...
async fn inner_test(
    ...
    recipient: Address,
    amount: u64,
    ledger_client: TestLedgerClient<(), (), InMemoryStorage<()>>,
    decoys: Vec<PolicyId>,
) {
    let actions = TxActions::v1().with_transfer(amount, recipient.clone(), PolicyId::Lovelace);

    let my_bal_before = ledger_client
        .balance_at_address(&signer, &PolicyId::Lovelace)
        .await
        .unwrap();
    let their_bal_before = ledger_client
